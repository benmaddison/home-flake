...
let
  ...
  move-mail = with pkgs; writeShellScriptBin "move-mail" ''
    set -euo pipefail
    ...
    exit 0
  '';
  notmuch-move = with pkgs; writeShellScriptBin "notmuch-move" ''
    set -euo pipefail
    ...
    exit 0
  '';
  notmuch-move-all = pkgs.writeShellScriptBin "notmuch-move-all" ''
    ${lib.concatStrings (lib.concatMap (account:
      lib.mapAttrsToList (tag: folder: ''
...
in {
  ...
  config = lib.mkIf (cfg != {}) {
    ...
    in lib.mapAttrs mkEmailAccount cfg;
    ...
    xdg.configFile = let
      ...
      in lib.foldl' (a: b: a // b) {} (map accountCfg accounts);
      mainCfg = let
        ...
        '';
      in {
        "neomutt/mailcap".text = with pkgs; ''
